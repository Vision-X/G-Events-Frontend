          ...
          <div>
            <div className="landing-btns mx-auto">
              <button
                ...
                className="floors-lg btn-primary basic-button"
              >
                4th Floor
              </button>
            </div>
        ...
        <ToggleDisplay show={this.state.selectedPortraitMap !== ""}>
          <div className="btn-group fixed-bottom}">
            <button
              onClick={this._onClick}
              className={
                "floors basic-button btn btn-secondary map-button " +
                (this.state.selectedFloor === "LL" ? "selected" : null)
              }
            >
              Lower Level
            </button>
            <button
              ...
              className={
                "floors basic-button btn btn-secondary map-button " +
                (this.state.selectedFloor === "L3" ? "selected" : null)
              }
            >
              3rd Floor
            </button>
            <button
              ...
              className={
                "floors basic-button btn btn-secondary map-button " +
                (this.state.selectedFloor === "L4" ? "selected" : null)
              }
            >
              4th Floor
            </button>
            ...
            <div>
              <button
                onClick={this._onClick}
                className={
                  "basic-button btn btn-secondary map-button rooms " +
                  (this.state.roomsDisplayed ? "selected" : null)
                }
              >
                Rooms
              </button>
          ...
          <div className="map-container">
            <img
              ...
              src={this.state.selectedPortraitMap}
              alt=""
            />
            <div className="">
              <img
                className="img-fluid landscape"
                src={this.state.selectedLandscapeMap}
                alt=""
              />
            </div>
          </div>
            ...
            <div className="room-list landing-btns mx-auto">
              {this.state.selectedFloorRooms.map(floorRoomName => {
                return (
                  <button
                    id={floorRoomName.id}
                    key={floorRoomName.id}
                    className="event-detail-card room mx-auto"
                    onClick={this._onClick}
                  >
